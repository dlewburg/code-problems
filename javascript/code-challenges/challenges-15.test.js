...
// This function was the biggest pain to figure out.
function screenForNames(names) {
  const regex = /^(Mr|Mrs|Ms|Dr)\. {1}[a-zA-Z\s]+$/;
...
let biggerThanLuke = (arr) => {
  ...
  const lukeMass = parseFloat(luke.mass);

  const heavierCharacters = arr.filter(character => parseFloat(character.mass) > lukeMass);

  const result = heavierCharacters.map(character => character.name).join(' - ');
...
const detectTicTacToeWin = (board) => {
  ...
  // Check diagonals
  if (check(0, 0, 1, 1, 2, 2) || check(0, 2, 1, 1, 2, 0)) {
    return true;
  }
