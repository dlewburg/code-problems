...
const addPurchases = (arr) => {
  return arr.reduce((accumulator, currentItem) => accumulator + currentItem.purchasePrice, 0);
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((accumulator, character) => {
    if (character.children) {
      accumulator += character.children.length;
    }
    return accumulator;
  }, 0);
};
...
const extractStats = (snorlaxData) => {
  return snorlaxData.stats.reduce((accumulator, currentStat) => {
    accumulator[currentStat.stat.name] = currentStat.baseStat;
    return accumulator;
  }, {});
};
...
const extractChildren = (arr) => {
  ...
  return charactersWithA.reduce((accumulator, currentCharacter) => {
    if (currentCharacter.children) {
      accumulator.push(...currentCharacter.children);
    }
    return accumulator;
  }, []);
};
