...
const standardizePhoneNumbers = (arr) => {
  const result = [];
  for (let i = 0; i < arr.length; i++) {
    const digitsOnly = arr[i].replace(/\D/g, ''); // replace all non-digits with empty string
...
const unenrollBrook = (arr) => {
  const result = [];
  for (let i = 0; i < arr.length; i++) {
    const courseRoster = arr[i].filter((name) => !name.includes('Brook'));
    result.push(courseRoster);
  }
  return result;
};
/* ------------------------------------------------------------------------------------------------
...
const sortByDay = (arr) => {
  let sortedEvents = daysOfWeek.map(() => []);

  arr.forEach(event => {
    daysOfWeek.forEach((day, index) => {
      if (event.includes(day)) {
        sortedEvents[index].push(event);
      }
    });
  });
