...
const count = (target, input) => {
  return input.reduce((acc, curr) => {
    const occurrences = curr.filter((num) => num === target).length;
...
let findMaleAndFemale = (data) => {
  const maleAndFemaleChars = data.filter((char) => char.gender === 'male' || char.gender === 'female');
  const names = maleAndFemaleChars.map((char) => char.name);
...
let findShortest = (data) => {
  const shortest = data.reduce((prev, curr) => {
