...
const hasChildrenValues = (arr, character) => {
  ...
  return characterObj && characterObj.children && characterObj.children.length > 0;
};
...
const houseSize = (arr) => {
  const sizes = arr.map(character => {
    const childrenCount = character.children ? character.children.length : 0;
    const membersCount = 1 + (character.spouse ? 1 : 0) + childrenCount;
...
const houseSurvivors = (arr) => {
  const survivors = arr.map(character => {
    const isSpouseDeceased = deceasedSpouses.includes(character.spouse);
    const childrenCount = character.children ? character.children.length : 0;
    const membersCount = 1 + (character.spouse && !isSpouseDeceased ? 1 : 0) + childrenCount;
