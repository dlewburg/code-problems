...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((ingredient) => {
    ...
    if (secondSpaceIndex !== -1) {
      result.push(ingredient.slice(secondSpaceIndex + 1));
    } else {
      result.push(ingredient.slice(firstSpaceIndex + 1));
    }
  });
...
const ingredientsList = listFoods(gruffaloCrumble);
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((ingredient) => {
    ...
    let foodName = words.slice(2).join(' ');
...
const foodsList = splitFoods(gruffaloCrumble);
...
const extractVowels = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    if (vowels.includes(str[i])) {
      extractedVowels.push(str[i]);
    } else {
      stringWithoutVowels += str[i];
    }
  }
  ...
  return [stringWithoutVowels, extractedVowels.join('')];
};
